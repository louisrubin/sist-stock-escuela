...
def insertar_datos(tabla_num):
    ...
    query1 = generar_query_insert(tabla_id= tabla_num)
...
def ver_stock(wea=""):
    if wea is None:
        ...
        stock = cursor.execute("SELECT nombre, stock FROM inventario").fetchall()
...
def ver_proveedores(wea=""):
    if wea is None:
        ...
        cursor = conexion.cursor()
        print("\n----------------------- PROVEEDORES -----------------------\n")
        print(cursor.execute("SELECT nombre, domicilio, telefono FROM proveedores").fetchall())
        print("--------------------------------------------------------")
        input("\n<continue>")
